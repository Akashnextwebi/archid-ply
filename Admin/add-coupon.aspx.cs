    ...
    public void GetAllCoupon()
    {
        try
        {
            ...
            foreach (CouponCode coupon in couponCodes)
            {
                strColors += @"<tr>
                                                <td>" + (i + 1) + @"</td>
                                                <td>" + coupon.CType + @"</td><td>" + coupon.CValue + @"</td><td>" + coupon.NoOfUsage + @"</td><td>" + coupon.CustomerId + @"</td><td>" + coupon.CCode + @"</td><td>" + coupon.ExpiryDate.ToString("dd/MMM/yyyy") + @"</td>
    ...
    public void GetCoupon()
    {
        try
        {
            ...
            if (coupon .Count>0)
            {
                ...
                txtCustomerId.Text = coupon[0].CustomerId;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid)
            {
                ...
                {
                    if (Convert.ToInt32(txtValue.Text) > 100)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "Snackbar.show({pos: 'top-right',text: 'Percentage of offer cannot exceed 100%',actionTextColor: '#fff',backgroundColor: '#ea1c1c'});", true);
                ...
                coupon.CustomerId = txtCustomerId.Text;
