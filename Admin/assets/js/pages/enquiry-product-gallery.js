...
$(document).ready(function () {
    ...
    $(document.body).on('click', '#AddToGallery', function () {
        ...
        $.ajax({
            url: "enquiry-product-gallery.ashx",
            type: "POST",
            data: data,
            contentType: false,
            processData: false,
       ...
       // var pid = getUrlParameter("id");
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'POST',
                    url: "enquiry-product-gallery.aspx/DeleteGallery",
                    data: "{id: '" + id + "'}",
...
function BindGalleryImages(id) {
    $.ajax({
        type: 'POST',
        url: "enquiry-product-gallery.aspx/GetGalleryImage",
        data: "{id: '" + id + "'}",
        contentType: 'application/json; charset=utf-8',
        dataType: "json",
        async: false,
        success: function (data2) {
            ...
            for (var i = 0; i < lnt.length; i++) {
                ...
                del += "<a href='javascript:void(0);' class='bs-tooltip btn btn-danger  deleteGalleryItem mt-3' data-id='" + lnt[i].Id + "' data-pid='" + lnt[i].ProductGuid + "' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'>";
                ...
                str = str + "<li data-id='" + lnt[i].Id + "' class='ui-state-default media  d-md-flex d-block text-sm-left text-end col-md-6'><div class='maindiv'>" + strtype + "<div><span>" + del + "</span></div></div></li>";
            }
...
function ArrengeImages(product) {
    var parameter = { "Id": product };

    $.ajax({
        type: 'POST',
        url: "enquiry-product-gallery.aspx/ImageOrderUpdate",
        ...
        success: function (data2) {
            if (data2.d.toString() == "Success") {
                ...
                Swal.fire({
                    title: "Oops...",
                    text: "Permission denied! Please contact to your administrator.",
                    icon: "error",
                    customClass: {
