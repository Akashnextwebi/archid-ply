        ...
        $.ajax({
            ...
            success: function (data2) {
                ...
                if (address.length > 0) {
                    ...
                    $('#txtBusinessName').text(address[0].CompanyName || "N/A");
...
function DeliverDateUpdate($this) {
    var OrderGuid = $this.attr("data-id");
    ...
    if (isValid) {
        var _data = { OrderGuid: OrderGuid, delDate: delDate };
        Swal.fire({
            title: "Are you sure?",
            text: "You won't be able to revert this!",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: "order-report.aspx/UpdateDeliveryDate",
                    ...
                    success: function (response) {
                        if (response.d === "Success") {
                            Swal.fire({
                                title: "Updated!",
                                text: "Delivery Date has been Updated.",
                                icon: "success",
...
function DispatchOrder($this) {
    ...
    if (isValid) {
        var _data = { OrderGuid: OrderGuid, courierName: courierName, trackingCode: trackingCode, trackingLink: trackingLink, oStatus: oStatus };
