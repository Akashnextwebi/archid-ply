        ...
        try
        {
            ...
            foreach (BannerImages img in imagess)
            {
                strImages += @"<tr>
                                                ...
                                                <td><img src='/" + img.MobileImage + @"' style='max-height:50px;' /></td>
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "Snackbar.show({pos: 'top-right',text: 'Desktop banner image size should be 1600*500 px',actionTextColor: '#fff',backgroundColor: '#ea1c1c'});", true);
                ...
                if (btnUpload.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conAP, pageName, "Edit", Request.Cookies["ap_aid"].Value))
                    {
                        ...
                        banner.MobileImage = UploadImage1();
