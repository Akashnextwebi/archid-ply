...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(conAP);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conAP = new SqlConnection(ConfigurationManager.ConnectionStrings["conAP"].ConnectionString);
            if (CreateUser.CheckAccess(conAP, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["ap_aid"].Value))
            {
                ...
                int exec = CreateUser.DeleteUser(conAP, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        {
            SqlConnection conAP = new SqlConnection(ConfigurationManager.ConnectionStrings["conAP"].ConnectionString);
            if (CreateUser.CheckAccess(conAP, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["ap_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conAP, id, HttpContext.Current.Request.Cookies["ap_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conAP, id, HttpContext.Current.Request.Cookies["ap_aid"].Value);
                    vali = "UnBlocked";
                }
