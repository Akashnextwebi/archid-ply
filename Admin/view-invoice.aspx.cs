...
public partial class Admin_view_invoice : System.Web.UI.Page
{
    ...
    public void BindOrders()
    {
        try
        {
            var oDetails = Reports.GetOrderDetails(conAP, Request.QueryString["id"]);
            if (oDetails.Rows.Count > 0)
            {
                double subTotal = 0, subTotalWithoutTax = 0, Discount = 0, tax = 0, totalAmount = 0;
                ...
                double.TryParse(Convert.ToString(oDetails.Rows[0]["Discount"]), out Discount);
                ...
                var oItems = Reports.GetOrderItems(conAP, Request.QueryString["id"]);
                if (oItems.Count > 0)
                {
                    for (int i = 0; i < oItems.Count; i++)
                    {
                        ...
                        strItems += @"<tr>
                                                                    <td>" + oItems[i].ProductName + @"</td>
                                                                    <td>₹" + price.ToString("##,###.00") + @"</td>
                                                                    <td class='text-center'>" + oItems[i].Quantity + @"</td>
                                                                    <td class='text-end'>₹" + finalAmount.ToString("##,###.00") + @"</td>
                                                                </tr>";
                    }
                }

                List<UserBillingAddress> billingAddress = UserCheckout.GetBillingAddress(conAP, Request.QueryString["id"]);
                List<UserDeliveryAddress> deliveryAddress = UserCheckout.GetDeliveryAddress(conAP, Request.QueryString["id"]);
