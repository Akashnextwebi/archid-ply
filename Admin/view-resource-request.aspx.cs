...
public partial class Admin_view_emailsubscribe : System.Web.UI.Page
{
    ...
    public void GetAllEmailRequests()
    {
        try
        {
            List<ResourceRequests> lcs = ResourceRequests.GetAllResourceRequests(conAP);
            int i = 0;
            foreach (ResourceRequests pro in lcs)
            {
                strRequests += @"<tr>
                                <td>" + (i + 1) + @"</td>                                           
                                <td>" + pro.Name + @"</td> 
 ...
 <td class='text-center'> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conAP = new SqlConnection(ConfigurationManager.ConnectionStrings["conAP"].ConnectionString);
            if (CreateUser.CheckAccess(conAP, "view-resource-request.aspx", "Delete", HttpContext.Current.Request.Cookies["ap_aid"].Value))
            {
                ResourceRequests BD = new ResourceRequests();
                BD.Id = Convert.ToInt32(id);
                int exec = ResourceRequests.DeleteResourceRequests(conAP, BD);
                if (exec > 0)
                {
                    x = "Success";
                }
