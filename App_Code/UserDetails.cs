...
public class UserDetails 
{
    ...
    public static int CreateUser(SqlConnection conAP, UserDetails userDetails)
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("insert into Customers (UserGuid,Gender,FirstName,LastName,EmailId,ContactNo,Pwd,Status,Subscribed,ForgotId,AddedOn,AddedIp,PassKey) values(@UserGuid,@Gender,@FirstName,@LastName,@EmailId,@ContactNo,@Pwd,@Status,@Subscribed,@ForgotId,@AddedOn,@AddedIp,@PassKey)", conAP);
            ...
            cmd.Parameters.AddWithValue("@Subscribed", SqlDbType.NVarChar).Value = userDetails.Subscribed;
            ...
            using (SqlCommand cmd = new SqlCommand(query, conAP))
            {
                ...
                details = (from DataRow dr in dt.Rows
                           select new UserDetails()
                           {
                               ...
                               Subscribed = Convert.ToString(dr["Subscribed"]),
    ...
    public static List<UserAddress> GetLoggedUserAddress(SqlConnection conAP, string uGuid)
    {
        ...
        try
        {
            string query = "Select cadd.*, c.ContactNo, c.EmailId,c.Pwd,c.Gender,c.ImageUrl from CustomerAddress as cadd inner join Customers as c on c.UserGuid = cadd.UserGuid where c.UserGuid=@UserGuid";
            using (SqlCommand cmd = new SqlCommand(query, conAP))
            {
                cmd.Parameters.AddWithValue("@UserGuid", SqlDbType.NVarChar).Value = uGuid;
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
    ...
    public static string ChangePassword(SqlConnection conAP, string UserGuid, string Pwd)
    {
        ...
        try
        {
            string query = "Update Customers Set Pwd=@Pwd  Where UserGuid=@UserGuid";
            using (SqlCommand cmd = new SqlCommand(query, conAP))
            {
                cmd.Parameters.AddWithValue("@Pwd", SqlDbType.NVarChar).Value = Pwd;
