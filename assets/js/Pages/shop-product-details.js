$(document).ready(function () {
    ...
    $(document.body).on("click", ".shop-up", function (e) {
        let quantity = parseInt($('#product_quantity').val(), 10);
        if (quantity <= 20) {
            $('#product_quantity').val(quantity);
            $('.btnAddToCartP').prop('disabled', false);
            $('.quantity-error').text("");
        } else {
            $('.quantity-error').text("");
            $('.btnAddToCartP').prop('disabled', true);
            $('.quantity-error').text("Quantity can't be greater than 20");
            return;
        }
    });
    $(document.body).on("click", ".shop-down", function (e) {
        let quantity = parseInt($('#product_quantity').val(), 10);
        if (quantity > 1) {
            $('.btnAddToCartP').prop('disabled', false);
            $('#product_quantity').val(quantity);
            ...
            $('.quantity-error').text("Quantity can't be less than 1");
            $('.btnAddToCartP').prop('disabled', true);
            return;
        }
    });
