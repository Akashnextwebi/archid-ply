            ...
            if (categories.Count > 0)
            {
                foreach (Category category in categories)
                {
                    ...
                    if (subCategories.Count > 0)
                    {
                        ...
                        {
                            subCat += @"<ul class='list-unstyled mb-0'><li><a href='/products-categories/" + category.CategoryUrl + @"' class='border-hover text-decoration-none py-3 d-block'><span class='border-hover-target'>" + subCategory.SubCategoryName + @"</span></a></li></ul>";
                        }
                    }
                    subCat = "<div class='col'><h6 class='fs-18px'>" + category.CategoryName + @"</h6>" + subCat + "</div>";
                    strDeskMenu += subCat;
                }
        ...
        try
        {
            ...
            if (ord != null)
            {
              ...
              /*  string Company = "";
                ...
                if (ord.Discount == "0" || ord.Discount == "" || ord.Discount == null)
                {
                    divDiscount.Visible = false;
                }
                else
                {
                    divDiscount.Visible = true; strDiscount = "- " + Convert.ToDecimal(ord.Discount).ToString("C", CultureInfo.CreateSpecificCulture("en-IN"));
                    strSubTotalWithDiscount = (Convert.ToDecimal(ord.SubTotal) + Convert.ToDecimal(ord.Discount)).ToString("C", CultureInfo.CreateSpecificCulture("en-IN"));
                    strTotalDiscount = Convert.ToDecimal(ord.Discount).ToString("C", CultureInfo.CreateSpecificCulture("en-IN"));
                }
                ...
                else
                {
                    ...
                    strTotalDiscount = (Convert.ToDecimal(ord.Discount) + Convert.ToDecimal(ord.AddDiscount)).ToString("C", CultureInfo.CreateSpecificCulture("en-IN"));
                }
